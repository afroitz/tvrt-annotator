...
import { getEmojiByName } from './utils/utils';
...
ipcMain.handle('loadSample', async (_event, fileName: string, index: number):Promise<AnnotationSample> => {
  ...
  sample.message_fwd_count = Number(sample.message_fwd_count)
  ...
  sample.message_reactions = emojiReactions;

  return sample as AnnotationSample;
});

/** Get basic info about an annotation file. Currently this is only the number of rows */
ipcMain.handle('getAnnotationFileInfo', async (_event, fileName: string): Promise<AnnotationInputFileInfo> => {
  const csvFilePath = path.join(app.getPath('userData'), 'annotation-input-data', fileName);
